...
<head>
    ...
    <title>Marcas — CARSINFINITY</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        body { font-family: 'Inter', sans-serif; background: #0d0d1a; min-height: 100vh; color: #fff; }
        .btn-edit{display:inline-flex;align-items:center;gap:0.3rem;padding:0.35rem 0.75rem;background:rgba(102,126,234,0.15);border:1px solid rgba(102,126,234,0.3);border-radius:7px;color:#a78bfa;text-decoration:none;font-size:0.78rem;font-weight:600;transition:all 0.15s}
        .btn-edit:hover{background:rgba(102,126,234,0.3);transform:translateY(-1px)}
        ...
        .orb-1 { width: 500px; height: 500px; background: #667eea; top: -200px; left: -100px; }
        .orb-2 { width: 350px; height: 350px; background: #764ba2; bottom: -100px; right: -100px; animation-delay: -6s; }
        ...
        .page-icon { width: 52px; height: 52px; background: linear-gradient(135deg,#667eea,#764ba2); border-radius: 14px; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; }
        .page-title h1 { font-size: 1.7rem; font-weight: 800; }
        .page-title p { font-size: 0.85rem; color: rgba(255,255,255,0.4); margin-top: 2px; }
        .new-btn { display: flex; align-items: center; gap: 0.5rem; padding: 0.7rem 1.4rem; background: linear-gradient(135deg,#667eea,#764ba2); border-radius: 10px; color: #fff; text-decoration: none; font-size: 0.9rem; font-weight: 600; transition: transform 0.15s, box-shadow 0.15s; }
        .new-btn:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(102,126,234,0.4); }
        ...
        .badge-count { background: rgba(102,126,234,0.2); border: 1px solid rgba(102,126,234,0.3); color: #a78bfa; padding: 0.2rem 0.7rem; border-radius: 20px; font-size: 0.78rem; font-weight: 600; }
        ...
        .pill { display: inline-block; padding: 0.25rem 0.75rem; border-radius: 20px; font-size: 0.78rem; font-weight: 500; background: rgba(102,126,234,0.15); border: 1px solid rgba(102,126,234,0.25); color: #a78bfa; }
...
<div class="wrapper">
    ...
    <div class="page-header">
        <div class="page-title">
            ...
            <div>
                <h1>Marcas</h1>
                <p>Listado completo de marcas registradas</p>
            </div>
        </div>
        <a href="{{ route('marca.create') }}" class="new-btn">+ Nueva Marca</a>
    </div>
    ...
    <div class="table-wrap">
        <div class="table-card">
            <div class="stats-bar">
                ...
                <span class="badge-count">{{ $marcas->count() }} marcas</span>
            </div>
            @if($marcas->isEmpty())
            <div class="empty">
                <div class="empty-icon">🏷️</div>
                <p>No hay marcas registradas aún.<br>¡Crea la primera!</p>
            </div>
            @else
            <table>
                <thead>
                ...
                <tbody>
                    ...
                    <tr>
                        <td><span class="pill">{{ $marca->id }}</span></td>
                        <td class="primary">{{ $marca->nombre }}</td>
                        <td>{{ $marca->pais ?? '—' }}</td>
                        <td>{{ Str::limit($marca->descripcion, 60) ?? '—' }}</td>
                        <td>
                            <div class="actions">
                                <a href="{{ route('marca.edit', $marca->id) }}" class="btn-edit">✏ Editar</a>
                                <form action="{{ route('marca.destroy', $marca->id) }}" method="POST" onsubmit="return confirm('¿Eliminar esta marca?')">
                                    @csrf @method('DELETE')
