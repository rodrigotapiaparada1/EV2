...
<head>
    ...
    <title>Clientes — CARSINFINITY</title>
    ...
    <style>
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .orb-1{width:500px;height:500px;background:#11998e;top:-200px;left:-100px}
        .orb-2{width:350px;height:350px;background:#38ef7d;bottom:-100px;right:-100px;animation-delay:-6s}
        ...
        .page-icon{width:52px;height:52px;background:linear-gradient(135deg,#11998e,#38ef7d);border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:1.5rem}
        .page-title h1{font-size:1.7rem;font-weight:800}
        .page-title p{font-size:0.85rem;color:rgba(255,255,255,0.4);margin-top:2px}
        .new-btn{display:flex;align-items:center;gap:0.5rem;padding:0.7rem 1.4rem;background:linear-gradient(135deg,#11998e,#38ef7d);border-radius:10px;color:#0d2b29;text-decoration:none;font-size:0.9rem;font-weight:700;transition:transform 0.15s,box-shadow 0.15s}
        .new-btn:hover{transform:translateY(-2px);box-shadow:0 8px 25px rgba(17,153,142,0.4)}
        ...
        .badge-count{background:rgba(17,153,142,0.15);border:1px solid rgba(17,153,142,0.3);color:#6ee7b7;padding:0.2rem 0.7rem;border-radius:20px;font-size:0.78rem;font-weight:600}
        ...
        .pill{display:inline-block;padding:0.25rem 0.75rem;border-radius:20px;font-size:0.78rem;font-weight:500;background:rgba(17,153,142,0.15);border:1px solid rgba(17,153,142,0.3);color:#6ee7b7}
        .avatar{width:34px;height:34px;border-radius:50%;background:linear-gradient(135deg,#11998e,#38ef7d);display:inline-flex;align-items:center;justify-content:center;font-size:0.85rem;font-weight:700;color:#0d2b29;flex-shrink:0}
        .client-row{display:flex;align-items:center;gap:0.75rem}
        .email-tag{color:rgba(255,255,255,0.45);font-size:0.85rem}
        .phone-tag{display:inline-block;padding:0.2rem 0.6rem;border-radius:6px;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.08);font-size:0.82rem;color:rgba(255,255,255,0.6)}
        .btn-edit{display:inline-flex;align-items:center;gap:0.3rem;padding:0.35rem 0.75rem;background:rgba(17,153,142,0.15);border:1px solid rgba(17,153,142,0.3);border-radius:7px;color:#6ee7b7;text-decoration:none;font-size:0.78rem;font-weight:600;transition:all 0.15s}
        .btn-edit:hover{background:rgba(17,153,142,0.3);transform:translateY(-1px)}
...
<div class="wrapper">
    ...
    <div class="page-header">
        <div class="page-title">
            ...
            <div><h1>Clientes</h1><p>Directorio de clientes registrados</p></div>
        </div>
        <a href="{{ route('cliente.create') }}" class="new-btn">+ Nuevo Cliente</a>
    </div>
    @if(session('success'))
    ...
    <div class="table-wrap">
        <div class="table-card">
            <div class="stats-bar">
                ...
                <span class="badge-count">{{ $clientes->count() }} clientes</span>
            </div>
            @if($clientes->isEmpty())
            <div class="empty"><div class="empty-icon">👤</div><p>No hay clientes registrados aún.</p></div>
            @else
            <table>
                <thead><tr><th>#</th><th>Cliente</th><th>Email</th><th>Teléfono</th><th>Acciones</th></tr></thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="client-row">
                                ...
                                <span class="primary" style="color:#fff;font-weight:600">{{ $c->nombre }}</span>
                            </div>
                        </td>
                        <td><span class="email-tag">{{ $c->email ?? '—' }}</span></td>
                        <td><span class="phone-tag">{{ $c->telefono ?? '—' }}</span></td>
                        <td>
                            <div class="actions">
                                <a href="{{ route('cliente.edit', $c->id) }}" class="btn-edit">✏ Editar</a>
                                <form action="{{ route('cliente.destroy', $c->id) }}" method="POST" onsubmit="return confirm('¿Eliminar este cliente?')">
                                    @csrf @method('DELETE')
