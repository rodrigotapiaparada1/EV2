...
<head>
    ...
    <title>Modelos — CARSINFINITY</title>
    ...
    <style>
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .orb-1{width:500px;height:500px;background:#f7971e;top:-200px;left:-100px}
        .orb-2{width:350px;height:350px;background:#ffd200;bottom:-100px;right:-100px;animation-delay:-6s}
        ...
        .page-icon{width:52px;height:52px;background:linear-gradient(135deg,#f7971e,#ffd200);border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:1.5rem}
        .page-title h1{font-size:1.7rem;font-weight:800}
        .page-title p{font-size:0.85rem;color:rgba(255,255,255,0.4);margin-top:2px}
        .new-btn{display:flex;align-items:center;gap:0.5rem;padding:0.7rem 1.4rem;background:linear-gradient(135deg,#f7971e,#ffd200);border-radius:10px;color:#1a1a2e;text-decoration:none;font-size:0.9rem;font-weight:700;transition:transform 0.15s,box-shadow 0.15s}
        .new-btn:hover{transform:translateY(-2px);box-shadow:0 8px 25px rgba(247,151,30,0.4)}
        ...
        .badge-count{background:rgba(247,151,30,0.15);border:1px solid rgba(247,151,30,0.3);color:#fcd34d;padding:0.2rem 0.7rem;border-radius:20px;font-size:0.78rem;font-weight:600}
        ...
        .pill{display:inline-block;padding:0.25rem 0.75rem;border-radius:20px;font-size:0.78rem;font-weight:500;background:rgba(247,151,30,0.12);border:1px solid rgba(247,151,30,0.25);color:#fcd34d}
        .marca-tag{display:inline-block;padding:0.2rem 0.6rem;border-radius:6px;font-size:0.78rem;background:rgba(255,255,255,0.07);border:1px solid rgba(255,255,255,0.1);color:rgba(255,255,255,0.6)}
        .btn-edit{display:inline-flex;align-items:center;gap:0.3rem;padding:0.35rem 0.75rem;background:rgba(247,151,30,0.15);border:1px solid rgba(247,151,30,0.3);border-radius:7px;color:#fcd34d;text-decoration:none;font-size:0.78rem;font-weight:600;transition:all 0.15s}
        .btn-edit:hover{background:rgba(247,151,30,0.3);transform:translateY(-1px)}
...
<div class="wrapper">
    ...
    <div class="page-header">
        <div class="page-title">
            ...
            <div><h1>Modelos</h1><p>Listado completo de modelos registrados</p></div>
        </div>
        <a href="{{ route('modelo.create') }}" class="new-btn">+ Nuevo Modelo</a>
    </div>
    @if(session('success'))
    ...
    <div class="table-wrap">
        <div class="table-card">
            <div class="stats-bar">
                ...
                <span class="badge-count">{{ $modelos->count() }} modelos</span>
            </div>
            @if($modelos->isEmpty())
            <div class="empty"><div class="empty-icon">🚗</div><p>No hay modelos registrados aún.</p></div>
            @else
            <table>
                <thead><tr><th>#</th><th>Nombre</th><th>Marca</th><th>Año</th><th>Acciones</th></tr></thead>
                <tbody>
                    @foreach($modelos as $m)
                    <tr>
                        <td><span class="pill">{{ $m->id }}</span></td>
                        <td class="primary">{{ $m->nombre }}</td>
                        <td><span class="marca-tag">{{ $m->marca->nombre ?? '—' }}</span></td>
                        <td>{{ $m->anio ?? '—' }}</td>
                        <td>
                            <div class="actions">
                                <a href="{{ route('modelo.edit', $m->id) }}" class="btn-edit">✏ Editar</a>
                                <form action="{{ route('modelo.destroy', $m->id) }}" method="POST" onsubmit="return confirm('¿Eliminar este modelo?')">
                                    @csrf @method('DELETE')
