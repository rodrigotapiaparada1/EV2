...
<head>
    ...
    <title>Vehículos — CARSINFINITY</title>
    ...
    <style>
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .orb-1{width:500px;height:500px;background:#4facfe;top:-200px;left:-100px}
        .orb-2{width:350px;height:350px;background:#00f2fe;bottom:-100px;right:-100px;animation-delay:-6s}
        ...
        .page-icon{width:52px;height:52px;background:linear-gradient(135deg,#4facfe,#00f2fe);border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:1.5rem}
        .page-title h1{font-size:1.7rem;font-weight:800}
        .page-title p{font-size:0.85rem;color:rgba(255,255,255,0.4);margin-top:2px}
        .new-btn{display:flex;align-items:center;gap:0.5rem;padding:0.7rem 1.4rem;background:linear-gradient(135deg,#4facfe,#00f2fe);border-radius:10px;color:#0f3460;text-decoration:none;font-size:0.9rem;font-weight:700;transition:transform 0.15s,box-shadow 0.15s}
        .new-btn:hover{transform:translateY(-2px);box-shadow:0 8px 25px rgba(79,172,254,0.4)}
        ...
        .badge-count{background:rgba(79,172,254,0.15);border:1px solid rgba(79,172,254,0.3);color:#7dd3fc;padding:0.2rem 0.7rem;border-radius:20px;font-size:0.78rem;font-weight:600}
        ...
        .pill{display:inline-block;padding:0.25rem 0.75rem;border-radius:20px;font-size:0.78rem;font-weight:500;background:rgba(79,172,254,0.12);border:1px solid rgba(79,172,254,0.25);color:#7dd3fc}
        .color-dot{display:inline-flex;align-items:center;gap:0.5rem}
        .dot{width:10px;height:10px;border-radius:50%;background:currentColor;opacity:0.7}
        .price-tag{color:#34d399;font-weight:600}
        .stock-tag{padding:0.2rem 0.6rem;border-radius:6px;font-size:0.78rem;font-weight:600}
        .stock-ok{background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.25);color:#34d399}
        .stock-low{background:rgba(251,191,36,0.1);border:1px solid rgba(251,191,36,0.25);color:#fbbf24}
        .stock-zero{background:rgba(248,113,113,0.1);border:1px solid rgba(248,113,113,0.25);color:#f87171}
        .btn-edit{display:inline-flex;align-items:center;gap:0.3rem;padding:0.35rem 0.75rem;background:rgba(79,172,254,0.15);border:1px solid rgba(79,172,254,0.3);border-radius:7px;color:#7dd3fc;text-decoration:none;font-size:0.78rem;font-weight:600;transition:all 0.15s}
        .btn-edit:hover{background:rgba(79,172,254,0.3);transform:translateY(-1px)}
...
<div class="wrapper">
    ...
    <div class="page-header">
        <div class="page-title">
            ...
            <div><h1>Vehículos</h1><p>Listado completo de vehículos en inventario</p></div>
        </div>
        <a href="{{ route('vehiculo.create') }}" class="new-btn">+ Nuevo Vehículo</a>
    </div>
    @if(session('success'))
    ...
    <div class="table-wrap">
        <div class="table-card">
            <div class="stats-bar">
                ...
                <span class="badge-count">{{ $vehiculos->count() }} vehículos</span>
            </div>
            @if($vehiculos->isEmpty())
            <div class="empty"><div class="empty-icon">🚙</div><p>No hay vehículos registrados aún.</p></div>
            @else
            <table>
                <thead><tr><th>#</th><th>Modelo</th><th>Color</th><th>Precio</th><th>Stock</th><th>Acciones</th></tr></thead>
                <tbody>
                    @foreach($vehiculos as $v)
                    <tr>
                        <td><span class="pill">{{ $v->id }}</span></td>
                        <td class="primary">{{ $v->modelo->nombre ?? '—' }}</td>
                        <td><span class="color-dot">{{ $v->color ?? '—' }}</span></td>
                        <td><span class="price-tag">${{ number_format($v->precio, 2) }}</span></td>
                        <td>
                            @php $s = $v->stock ?? 0 @endphp
                            <span class="stock-tag {{ $s > 5 ? 'stock-ok' : ($s > 0 ? 'stock-low' : 'stock-zero') }}">{{ $s }} uds.</span>
                        </td>
                        <td>
                            <div class="actions">
                                <a href="{{ route('vehiculo.edit', $v->id) }}" class="btn-edit">✏ Editar</a>
                                <form action="{{ route('vehiculo.destroy', $v->id) }}" method="POST" onsubmit="return confirm('¿Eliminar este vehículo?')">
                                    @csrf @method('DELETE')
